...
class Model(nn.Module):   
    ...
    def forward(self, inputs_ids_1,position_idx_1,attn_mask_1,inputs_ids_2,position_idx_2,attn_mask_2,labels=None): 
        ...
        # 从 3D attention_mask 创建标准的 2D attention_mask
        # 方法：如果该位置能关注到任何其他位置，就设为1
        attn_mask_2d = (attn_mask.sum(-1) > 0).float()
        outputs = self.encoder.roberta(inputs_embeds=inputs_embeddings,attention_mask=attn_mask_2d.unsqueeze(1),position_ids=position_idx,token_type_ids=position_idx.eq(-1).long())[0]
